...
| FORRT Outcome's `Repository URL` field | Points to `https://github.com/annefou/fiesta-galaxy-cellprofiler-eo`. |
...
| GHCR Docker pull | `docker pull ghcr.io/annefou/fiesta-galaxy-cellprofiler-eo:latest` over HTTPS. |
...
- [ ] Docker image builds and runs the snakemake target via `docker run --rm ghcr.io/annefou/fiesta-galaxy-cellprofiler-eo:latest`.
